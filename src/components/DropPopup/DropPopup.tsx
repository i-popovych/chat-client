...
export const DropPopup: FC<Props> = ({ onDrop, handleClose, isOpen }) => {
  const { getRootProps, getInputProps, isDragActive } = useDropzone({ onDrop, onDropAccepted });
  ...
  const dragZoneText = isDragActive ? 'Your files are here' : 'Put your files here';

  return (
    <Popup handleClose={handleClose} isOpen={isOpen}>
      <div className='flex items-center justify-center h-[270px] w-[600px] ' {...getRootProps()}>
        <input {...getInputProps()} />
